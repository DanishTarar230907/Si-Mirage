...
const SECTION_TEMPLATES = [
  ...
  { type: 'creativeShowcase', name: 'Creative Showcases', icon: ImageIcon },
  { type: 'categories', name: 'Silhouette Categories', icon: ImageIcon },
  { type: 'luxuryBanner', name: 'Luxury Capsule Banner', icon: ImageIcon },
  { type: 'trendingProducts', name: 'Trending Carousel', icon: Star },
  { type: 'everydayBanner', name: 'Everyday Essentials Banner', icon: ImageIcon },
  { type: 'mediaShowcase', name: 'Media Campaigns Showcase', icon: Video },
  { type: 'brandStory', name: 'Brand Editorial Story', icon: ImageIcon },
  { type: 'customVideo', name: 'Cinematic Video Hero', icon: Play },
  { type: 'testimonials', name: 'Customer Reviews', icon: Star },
  { type: 'gallery', name: 'Instagram Gallery Masonry', icon: ImageIcon },
...
export default function AdminHomePage() {
  ...
  const layout = cmsData.pageLayouts?.['/'] || [];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      ...
      {layout.map((section) => (
        <AdminSectionWrapper
          key={section.id}
          sectionId={section.type} // Edit content by component type identifier
          ...
          <div className="border border-[#C5A059]/30 bg-[#121212] p-8 text-left space-y-6 animate-fade-in shadow-2xl">
            <div className="flex justify-between items-center border-b border-white/10 pb-4">
              <div>
                ...
                <p className="text-[10px] text-white/50 tracking-wider uppercase mt-1">Select a pre-built section layout to add to the page</p>
              </div>
              <button
              ...
              {SECTION_TEMPLATES.map((tmpl) => {
                ...
                return (
                  <button
                    key={tmpl.type}
                    onClick={() => {
                      addSection('/', tmpl.type, tmpl.name);
                      setShowAddMenu(false);
                      alert(`Added "${tmpl.name}" to your workspace view! Save and publish to push live.`);
                    }}
