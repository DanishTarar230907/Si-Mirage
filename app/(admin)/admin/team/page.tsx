...
const SECTION_TEMPLATES = [
  { type: 'team', name: 'Atelier Team Grid', icon: ImageIcon },
...
export default function AdminTeamPage() {
  ...
  const layout = cmsData.pageLayouts?.['/team'] || [];
          ...
          <div className="border border-[#C5A059]/30 bg-[#121212] p-8 text-left space-y-6 animate-fade-in shadow-2xl">
            <div className="flex justify-between items-center border-b border-white/10 pb-4">
              <div>
                ...
                <p className="text-[10px] text-white/50 tracking-wider uppercase mt-1">Select a template to insert into the Team page layout</p>
              </div>
              <button
              ...
              {SECTION_TEMPLATES.map((tmpl) => {
                ...
                return (
                  <button
                    key={tmpl.type}
                    onClick={() => {
                      addSection('/team', tmpl.type, tmpl.name);
                      setShowAddMenu(false);
                      alert(`Added "${tmpl.name}" to your Team page! Save and publish to push live.`);
                    }}
